...
public:
    ...
    static Replay read(const std::vector<uint8_t>& data) {
        ...
        replay.gameVersion = static_cast<int32_t>(reader.readVarU64());
        replay.framerate = reader.readF64();
        replay.seed = static_cast<int32_t>(reader.readVarU64());
        replay.coins = static_cast<int32_t>(reader.readVarU64());
        replay.ldm = reader.readBool();
        replay.platformer = reader.readBool();
        replay.botInfo.name = reader.readStringVar();
        replay.botInfo.version = static_cast<int32_t>(reader.readVarU64());
        ...
        while (reader.remaining() > 0 && replay.inputs.size() < totalInputs) {
            ...
            if (replay.platformer) {
                delta = packed >> 3;
                button = static_cast<uint8_t>((packed >> 1) & 3);
            } else {
                delta = packed >> 1;
                button = 1;
            }
            ...
            if (hasExtension) {
                ...
                if (extLen > 0) {
                    ...
                    if (inputTag == "Phys") {
                        ...
                        input.physics = std::make_unique<PhysicsData>();
